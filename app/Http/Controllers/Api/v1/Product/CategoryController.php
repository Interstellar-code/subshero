...
class CategoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!empty($sort_str)) {
            ...
            $validator = Validator::make($sort_arr, [
                '*.column' => 'required|string|in:' . ProductCategory::$sortableColumns,
        ...
        // $query = ProductCategory::where('product_categories.user_id', Auth::id())
        $query = ProductCategory::where(function ($query) use ($search_query) {
        ...
        if (!empty($valid_sort_arr)) {
            foreach ($valid_sort_arr as $sort_item) {
                $query->orderBy("product_categories.{$sort_item['column']}", $sort_item['direction']);
            }
        }
    ...
    public function show(Request $request)
    {
        ...
        $data = ProductCategory::find($fields['id']);
    ...
    private function create(Request $request)
    {
        ...
        $fields = $request->validate([
            'name' => [
                'required',
                'string',
                'max:' . len()->product_categories->name,
    ...
    private function update(Request $request)
    {
        ...
        $row = ProductCategory::find($fields['id']);
        $row->name = $fields['name'];
        $row->save();
